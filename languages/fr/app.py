...
def send_email_to_buyer_via_mailjet(buyer_username, idea_name, message_subject):
    ...
    for idea in gift_ideas_data:
        if idea.get('bought_by') == buyer_username:
            ...
            if buyer_email:
                text_part = f"L'idée, '{idea_name}', a été supprimée mais vous l'avez déjà achetée."
    ...
    if idea:
        if not idea['bought_by']:
            ...
            if hide_purchaser or bought_anonymously:
                flash(f'"{idea["gift_name"]}" a déjà été acheté par un utilisateur anonyme.', 'warning')
            else:    
...
def change_avatar():
    ...
    if not avatar or not avatar_path.is_file():
        flash('Fichier avatar introuvable !', 'danger')
        return redirect(url_for('dashboard'))
    ...
    if request.method == 'POST':
        ...
        if 'new_avatar' in request.files and request.files['new_avatar'].filename == avatar:
            ...
            if avatar and not os.path.isfile(Path(app.config['AVATAR_DIR'], avatar)):
                flash('L\'avatar de l\'utilisateur généré a expiré et a été supprimé ! Essayez à nouveau pour ajouter le nouvel utilisateur.', 'error')
    ...
    if request.method == 'POST':
        ...
        if 'pool_name_to_delete' in request.form:
            ...
            if not is_valid_pool_name(pool_name):
                flash('Nom de pool invalide', 'error')
            ...
            if deleted:
                ...
                save_users(users)
                flash(f'Le pool "{pool_name}" a été supprimé', 'success')
            else:
                flash(f'Pool "{pool_name}" introuvable', 'error')
        ...
        try:
            ...
            flash(f' Pool "{pool_name}" créé', 'success')
            return redirect(url_for('secret_santa'))
        ...
        # Handle user details update
            ...
            if new_avatar_file and new_avatar_file.filename:
                ...
                try:
                    ...
                    flash('Avatar téléchargé avec succès !', 'success')
                except Exception as e:
                    flash(f'Erreur lors du chargement de l\'avatar : {str(e)}', 'error')
                    return redirect(url_for('manage_users'))
            else:
                ...
                if updated_avatar and updated_avatar != original_avatar:
                    ...
                    if not file_exists:
                        flash('L\'avatar de l\'utilisateur généré a expiré et a été supprimé ! Veuillez en générer un nouveau.', 'error')
                        return redirect(url_for('manage_users'))
...
def delete_old_gift_ideas_page():
    ...
    if request.method == 'POST':
        try:
            ...
            flash(f"{deleted_count} idées cadeau plus anciennes que le seuil ont été supprimées avec succès.", "success")
...
def delete_guest_user(username):
    ...
    if guest_user:
        ...
        flash(f'L\'utilisateur invité {username} a été supprimé avec succès ! {deleted_count} idées cadeaux achetées supprimées.', 'success')
    else:
