...
LOGIN_PAGE_MESSAGE='¿No tienes cuenta? Contacta con un miembro de tu familia para crear una cuenta.'
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'username' not in session:
            flash('Necesita iniciar sesión.', 'warning')
        ...
        if is_guest_user(session['username']):
            ...
            if not getattr(f, '_guest_allowed', False):
                flash('Esta función no está disponible para usuarios invitados.', 'danger')
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not user or not user.get('admin'):
            flash('Se requiere permiso de administración.', 'danger')
            return redirect(url_for('dashboard'))
...
def auth():
    ...
    if state != saved_state:
        flash("Error de autorización: estado inválido.", "danger")
        return redirect(url_for("login"))
    ...
    try:
        ...
        flash("Error de autorización OIDC.", "danger")
        return redirect(url_for("login"))
    ...
    try:
        ...
        flash("Error al analizar la información del usuario.", "danger")
        return redirect(url_for("login"))
    ...
    if user_in_db:
        ...
        flash("¡Inicio de sesión exitoso con OIDC!", "login_success")
        return redirect(url_for("dashboard"))
    ...
    if read_env_variable("ENABLE_AUTO_REGISTRATION", "false").lower() == "true":
        ...
        flash("Nuevo perfil creado. Por favor, completa tu configuración de perfil.", "info")
        return redirect(url_for("setup_profile"))  # Redirect to profile setup page

    flash("Usuario no encontrado y el registro está desactivado.", "danger")
    return redirect(url_for("login"))
...
def setup_profile():
    ...
    if user.get("avatar"):
        flash("La configuración del perfil no es necesaria. La imagen de perfil ya estaba configurada.", "info")
        return redirect(url_for("dashboard"))
    ...
    if request.method == "POST":
        ...
        flash("¡Configuración del perfil completada!", "success")
        return redirect(url_for("dashboard"))
...
def index():
    ...
    try:
        ...
        flash(f"Error de lectura users.json: {e}", 'danger')
        return redirect(url_for('setup'))
...
def login():
    ...
    login_message = read_env_variable("LOGIN_PAGE_MESSAGE") or "¿No tienes cuenta? Contacta con un miembro de tu familia para crear una cuenta."
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for user in users:
                if user['username'].lower() == input_username:
                    if user.get('shared_list', False):
                        flash('El usuario no existe', 'login_error')
                    ...
                    if verify_password(user['password'], password):
                        ...
                        flash('¡Sesión iniciada correctamente!', 'login_success')
                        return redirect(url_for('dashboard'))
                    else:
                        flash('Contraseña incorrecta', 'login_error')
            ...
            flash(f"Error de lectura users.json: {e}", 'login_error')
...
def register():
    ...
    if not read_env_variable('ENABLE_SELF_REGISTRATION', 'false').lower() == 'true':
        flash('El registro de nuevos usuarios no está activado. Póngase en contacto con un administrador.', 'danger')
        return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        if joining_code and submitted_joining_code != joining_code:
            flash('Código de invitación incorrecto. Compruébelo de nuevo.', 'danger')
            return render_template('register.html', joining_code=joining_code)
        
        # Validate required input
        if not username or not password or not full_name:
            flash('Por favor, rellena todos los campos obligatorios.', 'danger')
            return render_template('register.html', joining_code=joining_code)
        ...
        if any(user['username'].lower() == username for user in users):
            flash('El nombre de usuario ya existe. Por favor, elije un nombre diferente.', 'danger')
            return render_template('register.html', joining_code=joining_code)
        
        # Check for duplicate email only if email is provided
        if email and any(user.get('email', '').lower() == email.lower() for user in users):
            flash('Correo electrónico ya registrado. Por favor, utilice un correo electrónico diferente o póngase en contacto con el soporte técnico.', 'danger')
            return render_template('register.html', joining_code=joining_code)
        ...
        flash('¡Registro realizado correctamente! Ya puedes iniciar sesión.', 'success')
        return redirect(url_for('login'))
...
def add_idea(selected_user_id):
    ...
    if request.method == 'POST':
        ...
        flash(f'¡Idea "{name}" añadida a {user} por {added_by}!', 'success')
        return redirect(url_for('user_gift_ideas', selected_user_id=user))
    imgenabled = read_env_variable('IMGENABLED', 'true').lower() == 'true'
...
# Check if the idea was added by the current user, if it's in their list, OR if they're a member of the shared list
        ...
        if idea['added_by'] == current_user_username or idea['user_id'] == current_user_username or (shared_list and current_user_username in shared_list.get('list_members', [])):
            ...
            flash('No estás autorizado a eliminar esta idea.', 'danger')
    else:
...
def dashboard():
    ...
    if not current_user:
        flash('Datos de usuario no encontrados', 'danger')
        return redirect(url_for('login'))
...
def mark_as_bought(idea_id):
    ...
    if idea:
        if not idea['bought_by']:
            ...
            flash(f'¡"{idea["gift_name"]}" marcado como comprado!', 'success')
            ...
            flash(f'"{idea["gift_name"]}" ya ha sido comprado por {idea["bought_by"]}.', 'warning')
    else:
        flash('Idea no encontrada', 'danger')
...
def mark_as_not_bought(idea_id):
    ...
    if idea:
        ...
        if idea['bought_by'] == session['username']:
            ...
            flash(f'Marcado "{idea["gift_name"]}" como no comprado.', 'success')

            # Save the updated gift ideas back to the JSON file
            save_gift_ideas(gift_ideas_data)  # Save the updated list
        else:
            flash(f'No compraste "{idea["gift_name"]}", así que no puedes marcarlo como no comprado.', 'danger')
    else:
        flash('Idea no encontrada', 'danger')
...
def my_ideas():
    ...
    if not my_gift_ideas:
        flash('No has añadido ninguna idea de regalo.', 'info')
        return redirect(url_for('noidea'))
    # Ensure each idea has custom_fields and last_updated fields for template
...
def secret_santa_assignments():
    ...
    if not assigned_users:
        flash("Aún no tienes ningún Amigo Invisible.", "error")
        return redirect(url_for('secret_santa'))
        ...
        # Handle toggle admin
            ...
            for user in users:
                if user['username'] == username:
                    ...
                    flash('¡Estado de administrador actualizado!', 'success')
                    break
...
def edit_email_settings():
    if request.method == 'POST':
        ...
        try:
            ...
            flash("¡Ajustes de correo electrónico actualizados!", "success")
        except Exception as e:
            flash(f"Se ha producido un error: {e}", "danger")
    ...
    if request.method == 'POST':
        ...
        if new_message:
            try:
                ...
                flash("¡Mensaje de inicio de sesión actualizado!", "success")
            except Exception as e:
                flash(f"Error al actualizar el mensaje de inicio de sesión: {e}", "danger")
        else:
...
def delete_old_gift_ideas_page():
    ...
    if request.method == 'POST':
        try:
            ...
            flash(f"Se han eliminado correctamente {deleted_count} ideas de regalo más antiguas que el umbral.", "success")

            # Render the result to the template
            return render_template('delete_old_gift_ideas.html', deleted_count=deleted_count)

        except Exception as e:
            flash(f"Error al eliminar las ideas de regalo antiguas: {e}", "danger")
    ...
    if request.method == 'POST':
        ...
        if new_days:
            try:
                ...
                flash(f"El número de días para eliminar las viejas ideas de regalo ha sido actualizado a {new_days}.", "success")
                return redirect(url_for('change_delete_days'))

            except ValueError:
                flash("Número no válido de días introducidos. Introduce un número válido.", "danger")
        else:
            flash("Por favor, proporcione un valor para el número de días.", "danger")
...
def setup():
    ...
    if users_data:  # If there are any users in the file
        flash('Usuarios ya configurados. Página de configuración no disponible.', 'info')
    ...
    if request.method == 'POST':
        ...
        flash('Cuenta de administrador creada. Configure las variables de entorno.', 'success')
...
def setupenv():
    ...
    if not referer or '/setupadmin' not in referer:
        flash('Debe acceder a esta página desde la ruta setupadmin.', 'error')
    ...
    if request.method == 'POST':
        ...
        flash('¡Variables de entorno guardadas con éxito!', 'success')
...
def setup_oidc():
    if request.method == 'POST':
        ...
        try:
            ...
            flash('¡Ajustes OIDC guardados!', 'success')
            return redirect(url_for('setup_oidc'))  # Redirect to home page after saving
        except Exception as e:
            flash(f"Error al guardar los ajustes OIDC: {e}", "danger")
            return render_template('setup_oidc.html', current_values=oidc_env_variables)
    ...
    try:
        ...
        flash("Archivo de entorno no encontrado. Creando uno nuevo.", "warning")
...
def manage_groups():
    ...
    if request.method == 'POST':
        ...
        if new_group_name:
            ...
            flash('¡Nuevo grupo añadido!', 'success')
...
def update_group_assignments():
    ...
    flash('¡Tareas de grupo actualizadas con éxito!', 'success')
    return redirect(url_for('manage_groups'))
...
def manage_guest_users():
    ...
    if request.method == 'POST':
        ...
        save_users(users)
        flash('¡Usuario invitado creado!', 'success')
...
def delete_guest_user(username):
    ...
    if guest_user:
        ...
        flash(f'¡Usuario invitado {username} eliminado! {deleted_count} ideas compradas eliminadas.', 'success')
    else:
        flash('Usuario invitado no encontrado.', 'danger')
...
def guest_login():
    ...
    for user in users:
        if user.get('guest') and verify_password(user['password'], password):
            session['username'] = user['username']
            flash('¡Iniciada sesión como invitado!', 'login_success')
            return redirect(url_for('dashboard'))
    
    flash('¡Contraseña de invitado incorrecta!', 'login_error')
    return redirect(url_for('login'))
...
def update_currency_settings():
    ...
    flash('¡Ajustes de moneda actualizados! Por favor, reinicie para ver los cambios', 'success')
    return redirect(url_for('setup_advanced'))
...
def update_self_registration_settings():
    ...
    flash('¡Ajustes de registro actualizados!', 'success')
    return redirect(url_for('setup_advanced'))
...
def manage_shared_lists():
    ...
    if request.method == 'POST':
        ...
        save_users(users)
        flash(f'¡Lista compartida "{list_name}" creada!', 'success')
...
def delete_shared_list(list_username):
    ...
    if not shared_list:
        flash('Lista compartida no encontrada.', 'danger')
        return redirect(url_for('manage_shared_lists'))
    
    # Check if current user is the owner
    if shared_list.get('list_owner') != session['username']:
        flash('Solo el dueño de la lista puede eliminar esta lista compartida.', 'danger')
        return redirect(url_for('manage_shared_lists'))
    ...
    flash(f'¡Lista compartida "{shared_list["full_name"]}" borrada!', 'success')
    return redirect(url_for('manage_shared_lists'))
...
def edit_shared_list_members(list_username):
    ...
    # Check if current user is the owner
    if shared_list.get('list_owner') != session['username']:
        flash('Solo el dueño de la lista puede editar miembros.', 'danger')
        return redirect(url_for('manage_shared_lists'))
    ...
    flash(f'¡Miembros actualizados para "{shared_list["full_name"]}"!', 'success')
    return redirect(url_for('manage_shared_lists'))
...
def update_link_sharing():
    ...
    flash('¡Ajustes para compartir enlaces actualizados!', 'success')
    return redirect(url_for('setup_advanced'))
...
def manage_sharing():
    ...
    if not read_env_variable('ENABLE_LINK_SHARING', 'true').lower() == 'true':
        flash('La función de compartir enlaces está deshabilitada por el administrador.', 'danger')
        return redirect(url_for('dashboard'))
    ...
    if target_entity.get('shared_list'):
        ...
        if session['username'] not in target_entity.get('list_members', []):
            flash('No eres miembro de esta lista compartida', 'danger')
            return redirect(url_for('dashboard'))
    else:
        # For individual users, only the user themselves can manage sharing
        if target_username != session['username']:
            flash('Solo puedes administrar tus propias opciones de compartir', 'danger')
            return redirect(url_for('dashboard'))
    ...
    if request.method == 'POST':
        ...
        if 'create_link' in request.form:
            ...
            save_users(users)
            flash('¡Nuevo enlace para compartir creado!', 'success')
        ...
        # Toggle link status
            ...
            for link in target_entity['sharing']['public_links']:
                if link['token'] == token:
                    ...
                    flash(f'¡Enlace para compartir {status}!', 'success')
                    break
        ...
        # Extend expiration date
            ...
            for link in target_entity['sharing']['public_links']:
                if link['token'] == token:
                    ...
                    save_users(users)
                    flash(f'¡Enlace para compartir valido por {additional_days} días más! Nuevo vencimiento: {new_expiry.strftime("%Y-%m-%d")}', 'success')
                    break
        ...
        # Delete link
            ...
            save_users(users)
            flash('¡Enlace para compartir eliminado!', 'success')
